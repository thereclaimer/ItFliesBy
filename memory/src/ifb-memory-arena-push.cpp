...
ifb_memory::arena_push_step_2_push_bytes_relative(
    ...
    if (push_ref.result) {
        ...
        const ifb::u32 arena_index        = push_ref.context->handle_arena.h32;
        const ifb::u32 arena_size         = push_ref.cache.arena_size;
        const ifb::u32 arena_offset       = push_ref.cache.arrays.arena_position[arena_index];
        const ifb::u32 arena_position_new = arena_offset + push_ref.push_size;
...
ifb_memory::arena_push_step_2_push_bytes_absolute(
    ...
    if (push_ref.result) {
        ...
        if (push_ref.result) {
            ...
            //update the memory
            const ifb::addr arena_start           = push_ref.cache.arrays.arena_start[arena_index];
            const ifb::addr arena_memory_absolute = arena_start + arena_offset;
            push_ref.memory.absolute_pointer    = (ifb::ptr)arena_memory_absolute;
        }
    }
}