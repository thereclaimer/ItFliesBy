...
ifb_memory::arena_commit_step_1_cache_manager_properties(
    ...
    if (commit_ref.result) {
        ...
        //load the arrays
        ifb_memory::manager_load_arrays(memory_manager_internal,&commit_ref.manager_cache.arrays);
        
        //cache the other properties
        commit_ref.manager_cache.reservation_start = memory_manager_internal->reserved_memory_start;
        commit_ref.manager_cache.arena_count       = memory_manager_internal->count_arenas;
        commit_ref.manager_cache.arena_size        = memory_manager_internal->size_arena;
    
        //make sure the properties are non-zero
        commit_ref.result &= (commit_ref.manager_cache.reservation_start     != 0);
        commit_ref.result &= (commit_ref.manager_cache.arrays.arena_start    != NULL);
        commit_ref.result &= (commit_ref.manager_cache.arrays.arena_position != NULL);
...
ifb_memory::arena_commit_step_2_find_free_arena(
    ...
    if (commit_ref.result) {
        ...
        const IFBAddr* arena_start_array = commit_ref.manager_cache.arrays.arena_start;  
...
ifb_memory::arena_commit_step_3_commit_memory(
    ...
    if (commit_ref.result) {
        ...
        const IFBAddr start_reservation = commit_ref.manager_cache.reservation_start;
        ...
        commit_ref.manager_cache.arena_start = start_arena; 
...
ifb_memory::arena_commit_step_4_update_arrays(
    ...
    if (commit_ref.result) {
        ...
        //get arena values
        const IFBU32 arena_index = commit_ref.context->ids.arena;
        const IFBU32 arena_start = commit_ref.manager_cache.arena_start;

        //update the arrays
        commit_ref.manager_cache.arrays.arena_start    [arena_index] = arena_start;
        commit_ref.manager_cache.arrays.arena_position [arena_index] = 0;
    }
}