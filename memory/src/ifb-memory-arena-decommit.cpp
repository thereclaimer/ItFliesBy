...
ifb_memory::arena_decommit_step_0_validate_args(
    ...
    decommit_ref.result &= ifb_memory_macro_is_handle_valid_stack   (decommit_ref.context->handles.stack);
    decommit_ref.result &= ifb_memory_macro_is_handle_valid_manager (decommit_ref.context->handles.manager);
    decommit_ref.result &= ifb_memory_macro_is_handle_valid_arena   (decommit_ref.context->handles.arena);
}
...
ifb_memory::arena_decommit_step_1_cache_manager_properties(
    ...
    if (decommit_ref.result) { 
        ...
        IFBMemoryManager* memory_manager_internal = ifb_memory::manager_load_and_assert_valid(
            decommit_ref.context->handles.stack,
            decommit_ref.context->handles.manager);
        ...
        const IFBU32 arena_index = decommit_ref.context->handles.arena.h32;
