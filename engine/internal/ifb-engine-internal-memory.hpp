...
struct IFBEngineMemory {
    ...
    IFBEngineGlobalHandle global_handle_commit_array; // IFBCommit
};
...
namespace ifb_engine {
    ...
    const ifb_u32     memory_get_commit_page_count     (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id);
    const ifb_u32     memory_get_commit_size           (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id);
    const ifb_u32     memory_get_commit_offset         (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id);
    const ifb_address memory_get_commit_address        (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id);
    const ifb_ptr     memory_get_commit_pointer        (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id);
    const ifb_ptr     memory_get_commit_pointer        (const IFBEngineMemory* ptr_memory, const IFBCommitId commit_id, const ifb_u32 commit_offset);
    
    const ifb_u32     memory_align_size_to_page        (const IFBEngineMemory* ptr_memory, const ifb_size size);
    const ifb_size    memory_align_size_to_granularity (const IFBEngineMemory* ptr_memory, const ifb_size size);
    
    const IFBCommit*  memory_get_commit_array_pointer  (const IFBEngineMemory* ptr_memory);
