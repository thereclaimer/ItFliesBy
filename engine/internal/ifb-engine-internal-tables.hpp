...
struct IFBEngineTableColumns {
    ...
    ifb_handle_memory_t column_handles[IFB_ENGINE_TABLE_MANAGER_COLUMN_COUNT_MAX];
};
...
namespace ifb_engine {
    ...
    const ifb_handle_memory_t  table_manager_create (ifb_void);

    IFBEngineTableManager* table_manager_from_handle  (const ifb_handle_memory_t handle);
...
struct IFBEngineTableTag {
    ifb_handle_memory_t column_handle_tag_buffer;
    ifb_handle_memory_t column_handle_hash_value;
};

namespace ifb_engine {

    const ifb_b8        table_tag                          (IFBEngineTableTag& tag_table);
...
struct IFBEngineTableStackAllocator {
    ifb_handle_memory_t column_handle_arena_index;
    ifb_handle_memory_t column_handle_used;
};
...
struct IFBEngineTableArena {
    ...
    ifb_handle_memory_t column_handle_table_index_tag;
};

namespace ifb_engine {

    const ifb_b8               table_arena                               (IFBEngineTableArena& arena_table);
